...
layui.config({
	base: getRootPath_web()+'/static/ui/js/'
}).extend({
	dialog: 'dialog',
});
...
layui.use(['form', 'jquery', 'laydate', 'layer', 'laypage', 'dialog', 'element'], function() {
	...
	$('.addBtn').click(function() {
		...
		parent.page("添加", url, iframeObj, w = "800px", h = "650px");
		return false;
	}).mouseenter(function() {
	...
	$('#table-list').on('click', '.add-btn', function() {
		var url=$(this).attr('data-url');
		//将iframeObj传递给父级窗口
		parent.page("菜单添加", url, iframeObj, w = "800px", h = "650px");
		return false;
	})
	//列表删除
	...
	$('#table-list').on('click', '.edit-btn', function() {
		...
		//将iframeObj传递给父级窗口
		parent.page("编辑", url + "?id=" + id, iframeObj, w = "800px", h = "650px");
		return false;
	})
});
