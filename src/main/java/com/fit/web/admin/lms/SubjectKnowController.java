...
import com.fit.entity.LmsSubjectKnow;
import com.fit.entity.ZTreeNode;
import com.fit.service.LmsSubjectKnowService;
 ...
 */
@Controller
@RequestMapping("/admin/lms/subject/know")
public class SubjectKnowController extends BaseController {

    private static String PREFIX = "/admin/lms/subject/";

    @Autowired
    private LmsSubjectKnowService service;
    @Autowired
    ...
    public Object types(HttpServletRequest request) {
        ...
        List<LmsSubjectKnow> list = this.service.findList(params);
    ...
    public String editView(Long id, Model model) {
        if (OftenUtil.isNotEmpty(id)) {
            LmsSubjectKnow bean = this.service.get(id);
            model.addAttribute("bean", bean);
        }
        return PREFIX + "knowEdit";
    }
    ...
    @PostMapping("/save")
    @ResponseBody
    public Object save(LmsSubjectKnow bean) {
        LmsSubjectKnow entity = this.service.get(bean.getId());
    ...
    @ResponseBody
    @BizLog(logType = "MATERIAL_SET_STATE", logName = "修改状态", value = "更新ID：#{#id}")
    public Object changeState(Long id) {
        ...
        if (bean != null) {
            ...
            bean.setEtime(DateUtils.nowDate());
            bean.setEuser(userId);
    ...
    public AjaxResult tree() {
        List<ZTreeNode> tree = this.ztreeNodeService.subjectKnowZtree();
